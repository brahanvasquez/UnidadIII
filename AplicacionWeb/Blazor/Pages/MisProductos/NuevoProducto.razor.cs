    ...
    public partial class NuevoProducto
    {
        ...
        protected async Task Guardar()
        {
            ...
            if (productoExistente != null)
            {
                ...
                {
                    await Swal.FireAsync("Advertencia", "Ya existe un producto con este codigo", SweetAlertIcon.Warning);
                    return;
                }
            }
            ...
            if (inserto)
            {
                await Swal.FireAsync("Listo", "Producto guardado con exito", SweetAlertIcon.Success);
                _navigationManager.NavigateTo("/Productos");
            }
            else
            {
                await Swal.FireAsync("Error", "Producto no se pudo guardar", SweetAlertIcon.Error);
            }
        }
